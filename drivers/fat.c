...
/**
 * Determine the first logical block address for a given cluster number. There
 * are more than one sector per cluster so this function only returns the first
 * sector inside of the cluster.
 */
static inline uint32_t cluster_to_lba(uint32_t cluster)
{
	return part.cluster_begin_lba + ((cluster - 2) * part.sectors_per_cluster);
}
...
static fat_status_t find_dir_entry(char *name, uint32_t dir_cluster, fat_dir_entry_t *entry)
{
	...
	/* TODO: MAKE THIS WORK ACROSS CLUSTERS */
	const uint32_t current_sec = cluster_to_lba(dir_cluster);
...
{
	/* TODO: MAKE THIS WORK ACROSS CLUSTERS */
	const uint32_t current_sec = cluster_to_lba(dir_first_cluster);
